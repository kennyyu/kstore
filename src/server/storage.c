...
static
int
column_ids_sort(struct column_ids *ids, struct fetch_tuple **retftuples)
{
    assert(ids != NULL);
    assert(retftuples != NULL);
...
static
void
column_ids_fix(struct column_ids *ids, struct fetch_tuple *ftuples)
{
    assert(ids != NULL);
...
static
void
column_ids_cleanup(struct fetch_tuple *ftuples)
{
    if (ftuples == NULL) {
        free(ftuples);
    }
...
column_fetch(struct column *col, struct column_ids *ids)
{
    ...
    // sort the ids to make fetching faster
    TRY(result, column_ids_sort(ids, &ftuples), done);
    if (ids->cid_type == CID_BITMAP) {
    ...
    // fix the ids to make row alignment
    column_ids_fix(ids, ftuples);
    if (ids->cid_type == CID_ARRAY) {
        unsigned len = idarray_num(ids->cid_array);
        for (unsigned i = 0; i < len; i++) {
...
static
int
column_update_unsorted(struct column *col, struct column_ids *ids, int val)
{
    assert(col != NULL);
    assert(ids != NULL);
    ...
    while (cid_iter_has_next(&iter)) {
        ...
        assert(colentrybuf[requestedindex].ce_taken);
        colentrybuf[requestedindex].ce_val = val;
        dirty = true;
    }
    if (dirty) {
        TRY(result, file_write(col->col_base_file, curpage, colentrybuf), done);
...
int
column_update(struct column *col, struct column_ids *ids, int val)
{
    assert(col != NULL);
    assert(ids != NULL);
    ...
    case STORAGE_BTREE:
        ...
        goto done;
    case STORAGE_UNSORTED:
        TRY(result, column_update_unsorted(col, ids, val), done);
        break;
    }
